...
Each object class defined in the root
[README data model](../README.md#data-model) has one direct collection folder
below `data/`. Every JSON file in those collection folders represents one data
object and must use the object UUID as its filename:
...
content follows the root README data model: base-class fields are written
directly on the root object, not wrapped in a separate envelope.
