...
Group "1" --> "0..*" GroupPhase : has_phase
...
Membership "0..*" --> "0..2" Timepoint : anchors_timespan
Activity "0..*" --> "0..2" Timepoint : anchors_timespan
GroupPhase "0..*" --> "0..2" Timepoint : anchors_timespan
Source "1" --> "0..*" Attachment : has_attachment
Source --> Person : supports_claims_about
Source --> Group : supports_claims_about
```

Diagram edges are semantic/property-graph style: labels are verbs and arrow
direction is chosen to make the relationship read naturally. The canonical JSON
storage fields can point the other way; those fields are documented in the object
schemas and in the reference rules below. The source edges are examples; any
object or value object with `source_ids` can be supported by `Source`.

Compared to the current WPF prototype, `GroupType`, `Membership`, `Activity`,
and `GroupPhase` become first-class objects. This makes diffs, concurrent edits,
and conflict resolution easier because a change to one membership or type
definition does not rewrite an unrelated person or group file.
